...
function updateTutorialNav() {
    ...
    if (isFirst) {
        html += `<button class="tutorial-btn tutorial-btn-skip" onclick="closeTutorial()">Skip</button>`;
    } else {
    ...
    if (isLast) {
        ...
        html += `<button class="tutorial-btn tutorial-btn-next" onclick="nextTutorialSlide()">Next <i data-lucide="arrow-right"></i></button>`;
    }
...
function updateAdminTutorialNav() {
    ...
    if (isFirst) {
        html += `<button class="tutorial-btn tutorial-btn-skip" onclick="closeAdminTutorial()">Skip</button>`;
    } else {
    ...
    if (isLast) {
        ...
        html += `<button class="tutorial-btn tutorial-btn-next" onclick="nextAdminTutorialSlide()">Next <i data-lucide="arrow-right"></i></button>`;
    }
...
// Called by auth.js when admin clicks "Agenda Management"
...
const modalitiesData = {
    ...
    treino: [
        ...
        { id: 'tr_online', name: 'Online Training', icon: 'laptop', desc: 'Acompanhamento à distância grupal (60 min)' }
    ]
};
...
function updateBreadcrumbs(stepNumber) {
    ...
    if (stepNumber >= 2 && bookingData.category) {
        const catName = bookingData.category === 'osteopatia' ? 'Osteopathy' : 'Training';
...
function selectCategory(cat) {
    ...
    if (cat === 'osteopatia') {
        ...
        bookingData.serviceName = 'Osteopathy';
        goToStep(3);
        return;
    }
