                ...
                if (data && data.slots && data.slots[slotId]) {
                    ...
                    if (slotInfo.serviceType === 'grupal' && slotInfo.bookedUsers && slotInfo.bookedUsers.length > 0) {
                        ...
                        if (!isReadOnly) {
                            ...
                            cancelBtn.onclick = (e) => {
                                e.stopPropagation();
                                if (confirm(`Do you wish to delete the booking of "${displayName}"?`)) {
                        ...
                        if (isReadOnly) {
                            ...
                            if (slotInfo.status === 'available' || slotInfo.serviceType) {
                                ...
                                if (sType === 'grupal') {
                                    ...
                                    btn.innerHTML = `${hourStr}<br><span class="group-count">${count} bookings</span>`;
                                }
                            }
                    ...
                    if (sType === 'grupal') {
                        ...
                        if (userAlreadyBooked) {
                            ...
                            if (selectedClientSlots.some(s => s.id === slotId && s.action === 'cancel')) {
                                ...
                                btn.innerHTML = `${hourStr}<br><span style="font-size: 0.7rem; font-weight: 900; opacity: 1; display: block; margin-top: 2px;">CANCEL</span>`;
                            }
                        ...
                        // PERSONAL TRAINING or OSTEOPATHY
...
function adminCycleSlot(btn, hourStr) {
    if (!btn.classList.contains('active')) {
        ...
        btn.innerHTML = `${hourStr}<br><span class="group-count">0 bookings</span>`;
...
function openGroupPopup(slotId, slotInfo, db, weekId, userNames, isReadOnly = false) {
    ...
    titleEl.textContent = 'Online Training';
    subtitleEl.textContent = `${datePart} at ${timePart}`;

    listEl.innerHTML = '';
    const bookedUsers = slotInfo.bookedUsers || [];

    if (bookedUsers.length === 0) {
        listEl.innerHTML = '<div class="empty-group">No users booked.</div>';
    } else {
        bookedUsers.forEach(bu => {
            ...
            if (!isReadOnly) {
                ...
                item.querySelector('.remove-user-btn').onclick = async () => {
                    if (confirm(`Do you wish to remove "${displayName}" from this group training?`)) {
...
function setupAdminPublishButton(db, weekId, gridWrapper) {
    ...
    if (pubBtn) {
        ...
        newPubBtn.addEventListener('click', async () => {
            ...
            newPubBtn.textContent = 'Publishing...';
            ...
            newPubBtn.innerHTML = '<i data-lucide="send" style="width: 18px; height: 18px; margin-right: 8px;"></i> Publish Week';
            if (window.lucide) window.lucide.createIcons();
        });
    }
    ...
    if (clearBtn) {
        ...
        newClearBtn.addEventListener('click', async () => {
            if (confirm("Are you sure you want to remove all available (unbooked) time slots? Client bookings will not be affected.")) {
...
function toggleClientSlot(slotId, time, serviceType, btn, db, user, weekId, action = 'book', slotData = null) {
    ...
    if (index > -1) {
        ...
        if (action === 'cancel') {
            ...
            btn.innerHTML = `${time}<br><span style="font-size: 0.7rem; font-weight: 900; opacity: 1; display: block; margin-top: 2px;">CANCEL</span>`;
        }
    }
...
function updateBookingSummary(db, user, weekId) {
    ...
    if (bookings.length > 0) {
        ...
        title.textContent = "New Bookings";
    ...
    if (cancellations.length > 0) {
        ...
        title.textContent = "Cancellations";
        summaryList.appendChild(title);
    ...
    confirmBtn.textContent = `Confirm Changes (${selectedClientSlots.length})`;
...
function createSummaryItem(slot, isCancellation = false) {
    ...
    if (slot.serviceType === 'osteopatia') sName = 'Osteopathy';
...
async function cancelAdminBooking(db, weekId, slotId, slotInfo) {
    try {
        ...
        alert("Error removing booking: " + e.message);
    }
}
...
async function clearUnbookedSlots(db, weekId) {
    try {
        ...
        if (clearedCount === 0) {
            alert("There are no available time slots to clear.");
            return;
        }

        await updateDoc(docRef, updates);
        alert(`${clearedCount} available time slots were removed.`);
    } catch (e) {
        console.error(e);
        alert("Error clearing time slots.");
    }
}
